export default async function handler(req, res) {
  ...
  try {
    ...
    // GraphQL mutation to update item status to "Working on It" (index 8)
    const mutation = `
      mutation {
        change_column_value(
          board_id: ${boardId}, 
    ...
    console.log('Successfully updated Monday.com item status to Working on It:', { 
      itemId, 
      boardId, 
      response: data,
      mutation: mutation 
    ...
    res.status(200).json({
      success: true,
      message: 'Item status updated to Working on It'
    });
